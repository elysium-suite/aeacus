...
func scoreL(mc *metaConfig, id *imageData) {
	...
	for _, check := range mc.Config.Check {
		...
		for _, condition := range check.Pass {
			status = processCheckL(mc, &check, condition.Type, condition.Arg1, condition.Arg2)
			if status {
				break
			}
		}
		for _, condition := range check.Fail {
			failStatus = processCheckL(mc, &check, condition.Type, condition.Arg1, condition.Arg2)
			if failStatus {
				status = false
				break
			}
...
func processCheckL(mc *metaConfig, check *check, checkType string, arg1 string, arg2 string) bool {
	switch checkType {
	case "Command":
		if check.Message == "" {
